...
Private Const GAP_MM As Double = 8#               ' Зазор между видами [cite: 46]
Private Const LAYOUT_PAD_MM As Double = 6#        ' Отступ от границ листа [cite: 46]
Private Const ORTHO_SCALE_MARGIN As Double = 0.95 ' Запас при подборе масштаба [cite: 46]
Private Const MAX_AUTO_SCALE As Double = 8#       ' Максимальный масштаб (1:8) [cite: 46]
Private Const DIM_OFFSET_MM As Double = 7#        ' Отступ размерной линии [cite: 136]
...
Public Sub BuildOrUpdateAlbumCore(ByVal oDoc As DrawingDocument, ByVal modelItems As Collection)
    ...
    On Error GoTo EH
    ' Запуск "тихой" операции для скорости [cite: 117]
    ...
    For i = 1 To modelItems.Count
        ...
        ' Создание листа на основе формата [cite: 117]
        Set oSheet = CreateAlbumSheet(oDoc, modelPath) 
        
        ' Открытие 3D-модели камня [cite: 118]
        Set oModelDoc = ThisApplication.Documents.Open(modelPath, False)
        
        ' РАЗМЕЩЕНИЕ ВИДОВ: Главный алгоритм [cite: 123, 129]
        BuildSheetViews oDoc, oSheet, oModelDoc
        
        ' АВТО-ОБРАЗМЕРИВАНИЕ: Простановка габаритов [cite: 132]
...
CleanUp:
    ...
    Exit Sub
EH:
    Debug.Print "LOG: Album build failed. Err=" & Err.Number [cite: 119]
    Resume CleanUp
End Sub
...
Private Function ResolveTechViewLayout(ByVal oDoc As DrawingDocument, ByVal oSheet As Sheet, ByVal oModelDoc As Document) As Object
    ' Здесь происходит расчет, чтобы виды не перекрывали штамп [cite: 53, 123]
    ' Используется итерационный подбор: масштаб уменьшается на 3%, пока виды не влезут [cite: 58, 130]
...
Private Sub AutoDimensionOrthographicView(ByVal oDoc As DrawingDocument, ByVal oSheet As Sheet, ByVal oView As DrawingView, ByVal viewKey As String)
    ' Поиск крайних кривых на чертеже [cite: 61, 132]
    ' Простановка горизонтальных и вертикальных габаритов [cite: 63, 134]
End Sub
